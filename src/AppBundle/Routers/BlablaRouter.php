...
use AppBundle\Connectors\BlablaConnector;
use AppBundle\Entity\BookBlabla;
use AppBundle\Entity\ExternalCityBlabla;
use AppBundle\Entity\ExternalStationBlabla;
use AppBundle\Entity\ExternalTariffBlabla;
use AppBundle\Entity\ExternalTicketBlabla;
...
use AppBundle\Exceptions\BlablaConnectorException;
...
use AppBundle\VO\BlablaBookRoute;
use AppBundle\VO\BlablaBookRoutePerson;
use AppBundle\VO\ExternalRouteBlabla;
...
class BlablaRouter
{
	...
	public function __construct(BlablaConnector $connector,
	...
	public function findRoutes(SearchExternal $searchExternal)
	{
		...
		$fromList = $routeFilter->getFromCity()->getExternalCitiesByType(ExternalRouter::BLABLA);
		$toList = $routeFilter->getToCity()->getExternalCitiesByType(ExternalRouter::BLABLA);

		if ($fromList->isEmpty() or $toList->isEmpty() === null) return [];

		/** @var ExternalCityBlabla $fromExternalCity */
		/** @var ExternalCityBlabla $toExternalCity */
		foreach ($fromList->toArray() as $fromExternalCity) {
			foreach ($toList->toArray() as $toExternalCity) {
				...
				} catch (BlablaConnectorException $e) {
					continue;
				}
	 ...
	 * @throws \Exception
	 */
	private function createRoute(SearchExternal $searchExternal, $routeData, $direction, ExternalCityBlabla $fromExternalCity, ExternalCityBlabla $toExternalCity)
	{
		$route = new Route;
	...
	private function createFareForRoute(Route $route, $tariffData)
	{
		$externalTariff = $this->externalTariffService->getExternalTariffByIdent(
			$tariffData->code, ExternalRouter::BLABLA
		);

		$language = $this->languageService->getEnglish();

		if ($externalTariff === null) {
			$externalTariff = new ExternalTariffBlabla;
	...
	public function syncExternals()
	{
		...
		foreach ($cities as $c) {
			$externalCity = $this->externalCityService->getExternalCityByIdent($c->id, ExternalRouter::BLABLA);
			if ($externalCity === null) {
				$externalCity = new ExternalCityBlabla;
		...
		foreach ($stations as $s) {
			$externalStation = $this->externalStationService->getExternalStationByIdent($s->id, ExternalRouter::BLABLA);
			if ($externalStation === null) {
				$externalStation = new ExternalStationBlabla;
				$externalStation->setIdent($s->id);
				$externalStation->setExternalCity(
					$this->externalCityService->getExternalCityByIdent($s->city, ExternalRouter::BLABLA)
				);
			}
	...
	{
		/** @var ExternalRouteBlabla $externalObject */
		$externalObject = $route->getExternalObject();

		/** @var Order $order */
		$order = $route->getBooks()->map(function (BookBlabla $book) {
			return $book->getOrder();
		})->first();

		$bookRoute = new BlablaBookRoute;
		...
		$order->getOrderPersonRouteTariffs()->map(function (OrderPersonRouteTariff $orderPersonRouteTariff) use ($externalObject, $order, $bookRoute) {
			...
			$bookRoute->addPerson($person = new BlablaBookRoutePerson);
		...
		$route->getBooks()->map(function (BookBlabla $book) use ($bookRoute) {
	...
	{
		$externalObject = $route->getExternalObject();
		if ($externalObject instanceof ExternalRouteBlabla) {
		...
		$externalObject = new ExternalRouteBlabla;
