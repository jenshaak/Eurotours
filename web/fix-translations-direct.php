<?php
// Fixed script using entity manager directly
...
echo "<h1>Fix English Translations - Direct Entity Manager</h1>";
...
try {
    ...
    echo "<h2>Processing $totalCount cities with direct entity manager...</h2>";
    ...
    foreach ($testCities as $testCityName) {
        foreach ($cities as $city) {
            if ($city->getName()->getString($czechLanguage) === $testCityName) {
                // Refresh the entity from database to get latest data
                $em->refresh($city);
