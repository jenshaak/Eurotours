...
RUN apt-get update && \
    ...
    docker-php-ext-install pdo pdo_mysql zip opcache soap gd pcntl
...
COPY ./docker/php.dev.ini /usr/local/etc/php/conf.d/eurotours-php.ini
...
WORKDIR /var/www/app
COPY . /var/www/app
COPY ./docker/run.sh /var/www/app/run.sh
